	...
	private String ignoreAction(int rollD100, int rollD12) {
		if (!percentTest(rollD12*5+20)) {
			if (!percentTest(rollD100)) {
				...
				return "Stai per ignorare il luogo, quando tutte le candele si spengono contemporaneamente, alcune figure "
			...
			return "Ti allontani in fretta dal luogo del rituale, mentre te ne vai senti un canto provvenire da l&agrave;. "
					+ "Accelleri il passo impaurita e ti allontani il pi&ugrave; in fretta possibile.";
		} else {
			return "Lasci la stanza misteriosa, meglio non averci niente a che fare.";
		}
	...
	private String completeAction(int rollD100, int rollD12) {
		if (rollD12 < 7) {
			if (!percentTest(rollD100/2)) {
				...
				return "Tenti goffamente di completare il rituale, ma sbagli diversi passaggi e questo non &egrave; "
				...
				return "Tenti goffamente di completare il rituale, ma alcuni passaggi sono poco chiari e non "
			...
			if (!percentTest(rollD100 + rollD12)) {
				...
				if (playerInventoryDB.getGold() > 10L) {
					...
					return response + "Sborsi subito <b>10 monete</b> al demone e questo risponde: <i>'Il tuo tributo "
					...
					return response + "Dici di non possedere molto oro e il demone risponde con tono arrabbiato.<br />"
				...
				return "Segui i passaggi descritti nel libro come una ricetta di cucina, ma alla fine non accade niente.<br />"
				...
				if (playerInventoryDB.getGold() > 6L) {
					...
					return response.concat(String.format("Posizioni le monete al centro del cerchio e queste iniziano a fondere sprigionando un "
					...
					return response.concat("Controlli nelle tasche, ma non trovi oro sufficiente... Tristemente abbandoni "
			...
			if (!percentTest(rollD100 + rollD12) && !percentTest(rollD100 - rollD12)) {
				...
				return "Attivi il portale con facilit&agrave;. Posizioni come ultima cosa una candela e questa inizia "
				...
				playerInventoryDB.addItems(item);
				return String.format("Attivi il portale con facilit&agrave;. Posizioni come ultima cosa una candela "
	...
	private String destroyAction(int rollD100, int rollD12) {
		if (rollD12 < 7 && !percentTest(rollD12*4)) {
			if (!percentTest(rollD100)) {
				adventureDB.decreasePlayerHealth();
				adventureDB.decreasePlayerHealth();
				return "Attacchi subito il libro, ma appena lo colpisci questo esplode. E di conseguenza grosse rocce "
						+ "cadono dal soffitto, sei investita dalla frana e vieni ferita gravemente";
			} else {
				return "Attacchi subito il libro, ma appena lo colpisci questo esplode. Con agilit&agrave; ti copri dal "
			...
			if (percentTest(rollD100 + rollD12)) {
				...
				return "Attacchi il rituale, ma questo esplode in un onda di energia oscura che ti infonde una "
						+ "maledizione magica.<br />"
						+ "-Nuovo effetto attivo-";
			} else if (percentTest(rollD100 + 10)) {
				adventureDB.decreasePlayerHealth();
				return "Distruggi il rituale. Appena hai finito vedi dei loschi figuri incappucciati arrivare. "
				...
				return "Distruggi il leggio e il rituale. Quando hai finito una magia oscura si libera in aria e si dissolve. "
			...
			if (!percentTest(rollD100+rollD12)) {
				return "Attacchi il rituale e distruggi ogni cosa. Qualunque cosa stessero facendo qui, ora non la potranno "
						+ "pi&ugrave; fare";
			} else {
				adventureDB.increasePlayerHealth();
				adventureDB.increasePlayerHealth();
				return "Attacchi il rituale e distruggi ogni cosa. Questo libera delle anime imprigionate nel cerchio "
