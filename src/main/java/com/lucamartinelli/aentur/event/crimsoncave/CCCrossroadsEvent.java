...
public class CCCrossroadsEvent implements EventAction {
	...
	private String leftAction(int rollD100, int rollD12) {
		if (rollD12 < 4) {
			if (percentTest(rollD100)) {
				return "Apri la porta e segui il corridoio, piano piano il liquido rosso scompare fino a sparire. Segui "
				...
				return String.format("Entri nel corridoio, le pareti sono imbrattate di liquido rosso, dopo qualche passo vedi un "
			...
			if (percentTest(rollD100)) {
				return "Entri nel corridoio e segui la scia di liquido a terra. Dopo qualche metro entri in una stanza con"
			...
			if (percentTest(rollD100)) {
				...
				return String.format("Entri nel corridoio e segui la scia di liquido rosso. Arrivi in una stanza dove si deve essere "
				...
				return String.format("Entri nel corridoio e segui la scia di liquido rosso fino a raggiungere la fonte delle tracce. Vedi un "
				...
				return "Entri nel corridoio e segui la scia di liquido rosso fino a raggiungere la fonte delle tracce. Vedi un "
						+ "topino ferito accasciato a terra. Immediatamente cerchi di aiutarlo applicando delle bende sulla "
	...
	private String centralAction(int rollD100, int rollD12) {
		if (rollD12 < 6) {
			if (percentTest(rollD100)) {
				return "Entri nel corridoio con la strada battuta. Il corridoio &egrave; largo e comodo da percorrere.<br/>"
						+ "Incredibilmente non trovi nessuna minaccia, ma solo una strada sgombra. Continui per la tua via.";
			} else {
				adventureDB.decreasePlayerHealth();
				return "Entri nel corridoio con la strada battuta. Percorri un comodo corridoio che ti porta in una stanza "
			...
			if (percentTest(rollD100 + rollD12)) {
				...
				return "Entri nel corridoio con la strada battuta. Lungo il corridoio trovi una stanza che funge da "
			...
			if (percentTest(rollD100 + rollD12)) {
				...
				return String.format("Entri nel corridoio con la strada battuta. Dopo qualche metro ti ritrovi in una stanza buia, con la luce "
	...
	private String rightAction(int rollD100, int rollD12) {
		if (rollD12 < 3) {
			if (percentTest(rollD100*2-10)) {
				return "Entri nel corridoio, &egrave; stretto e lungo. Man mano cammini senti il rumore dello scorrere "
				...
				adventureDB.decreasePlayerHealth();
				return "Entri nel corridoio, &egrave; stretto e lungo. Man mano cammini senti il rumore dello scorrere "
						+ "dell'acqua sempre pi&ugrave; forte. Ad un certo punto guardi in intorno a te per capire meglio "
			...
			if (percentTest(rollD100+rollD12/2)) {
				...
				playerInventoryDB.addItems(item);
				return String.format("Percorri il corridoio, che non &egrave; poi tanto lungo, arrivi in una stanza con "
				...
				if (percentTest(rollD100+rollD12)) {
					if (percentTest(rollD100+rollD12)) {
						...
						return "Percorri il corridoio, che non &egrave; poi tanto lungo, arrivi in una stanza con "
								+ "un laghetto in mezzo e una cascatina sgorga dalla parete.<br/>"
					...
					if (!percentTest(rollD100+rollD12)) {
						adventureDB.decreasePlayerHealth();
						return "Percorri il corridoio e noti che attorno a te c'&egrave; del liquido che sgorga dalle "
